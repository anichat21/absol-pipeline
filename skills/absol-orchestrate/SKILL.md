...
|---|---|---|
| `shape:` | if intent is genuinely ambiguous, invoke `absol-shaper` inline; if the request context already settles it, skip — shape isn't ceremony | **skip the item**, report "needs shaping: <the open question>" |
...
**Retry loop** — on `verification_result: fail` or `task-failed`: count this task's
`task-retry` events; if < 2, spawn `absol-planner` with the task + failure event, append
`task-retry` with its one-line amendment, re-execute. At 2:
...
- afk → log it, mark the task failed, continue to the next task.
...
rewrites state.md, deletes run.md. Relay its report verbatim; if anything failed, is blocked,
or owes smoke, that's the headline, not a footnote.
