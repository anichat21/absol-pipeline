...
The orchestrator passes you a **filtered** subset — just the relevant `[job]` entries and their matching `[task]` definitions. Don't parse the full `todo-run.md`; you've been handed the targets already.
...
- `.absol/adr/` — every ADR in or near the touched subsystem; cross-reference architectural choices
- `state.md` — project context
- Source code — the actual files in `files_touched` and their close neighbours (this is a deep review)
...
The orchestrator already handed you the jobs to review and matching tasks. Read each task's acceptance criteria and verification steps.
...
- Use CONTEXT.md vocabulary when naming modules and concepts. Don't drift.
