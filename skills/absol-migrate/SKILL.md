...
- Read-only until the user approves Step 4. Steps 1–3 don't write.
- Doesn't touch code, non-absol MD files, or git (no auto-commit). The diff is left for the user.
- One project at a time. No batch mode.
- Pipeline churn (plan/run state) is regenerated by the next run, not preserved.
